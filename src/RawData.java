...
public class RawData extends javax.swing.JFrame {
    ...
    public RawData(List<Packet> p, int index, PcapHandle handle) {
        ...
        if (p != null && index >= 0 && index < p.size()) {
            byte[] rawData = p.get(index).getRawData();
            if (rawData != null) {
                jTextArea1.setText(Arrays.toString(rawData));
            } else {
                jTextArea1.setText("No raw data available for this packet");
            }
        } else {
    ...
    private void initComponents() {
        ...
        // Gradient Header
        JPanel headerPanel = UITheme.createGradientHeader("🔢  RAW DATA");

        // Terminal-styled Raw Data Display
        ...
        JPanel borderedContent = UITheme.createTitledPanel("Raw Data Content", scrollPane);
